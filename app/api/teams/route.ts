...
import { generateTeamLogo } from "@/lib/dicebear";
import {
  validateQueryParams,
  validateFormData,
  validateRequest,
...
export async function POST(req: NextRequest) {
  ...
  let teamNameForLog: string | undefined;
  ...
  try {
    ...
    let captain: string;
    let players: string[];
    let teamImage: Blob | undefined;

    if (contentType.includes("application/json")) {
      const body = await req.json();
      const validation = validateRequest(createTeamJsonBodySchema, body);
      if (!validation.success) {
        return validation.error;
      }
      ({ name, city, captain, players } = validation.data);
      teamImage = undefined;
    } else {
      ...
      ({ name, city, captain, players, teamImage } = validation.data);
    }
    teamNameForLog = name;
    ...
    // Upload team image to Cloudinary if provided, otherwise use Dicebear fallback
    let logoUrl: string;
    if (teamImage) {
      ...
      // Generate fallback logo using Dicebear
      logoUrl = generateTeamLogo(name);
    }

    // Format players for schema
    const formattedPlayers = playerDocs.map((p) => ({
      user: p._id,
      role: p._id.toString() === captain ? "captain" : "player",
    }));
    ...
    logError(error, {
      ...
      ...(teamNameForLog && { extra: { teamName: teamNameForLog } }),
    });
