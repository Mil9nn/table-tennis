...
import { validateHybridConfig } from "@/services/tournament/core/phaseManagementService";

function isRetryableWriteConflict(error: any): boolean {
  ...
  return (
    ...
    message.includes("Write conflict during plan execution")
  );
}
...
export async function POST(
  ...
  try {
    ...
    // Hard-stop invalid hybrid configuration before generation starts.
    // Keep this explicit in-route check (in addition to service validation)
    // so config-shape drift cannot bypass validation.
    if (tournament.format === "hybrid") {
      const participantCount = Array.isArray((tournament as any).participants)
        ? (tournament as any).participants.filter((p: any) => p != null).length
        : 0;
      const effectiveUseGroups =
        Boolean((tournament as any).hybridConfig?.roundRobinUseGroups) ||
        Boolean((tournament as any).useGroups);
      const effectiveGroupCount = Number(
        (tournament as any).hybridConfig?.roundRobinNumberOfGroups ||
          (tournament as any).numberOfGroups ||
          0
      );

      if (effectiveUseGroups || effectiveGroupCount > 0) {
        if (effectiveGroupCount < 2) {
          return NextResponse.json(
            { error: "Number of groups must be at least 2" },
            { status: 400 }
          );
        }
        if (effectiveGroupCount >= participantCount) {
          return NextResponse.json(
            { error: "Number of groups must be less than total participants" },
            { status: 400 }
          );
        }
        if (participantCount < effectiveGroupCount * 2) {
          return NextResponse.json(
            {
              error: `Need at least ${effectiveGroupCount * 2} participants for ${effectiveGroupCount} groups (minimum 2 per group)`,
            },
            { status: 400 }
          );
        }
      ...
      const hybridValidation = validateHybridConfig(tournament as any);
      if (!hybridValidation.isValid) {
        const message =
          hybridValidation.errors[0] ||
          "Invalid hybrid tournament configuration";
        return NextResponse.json(
          {
            error: message,
            details: hybridValidation.errors.join(" | "),
          },
          { status: 400 }
        );
      }
    ...
    // Retry on transient write conflicts from multi-document transactions.
    let result: Awaited<ReturnType<typeof matchGenerationOrchestrator.generateTournamentDraw>> | null = null;
    const maxAttempts = 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        result = await matchGenerationOrchestrator.generateTournamentDraw(
          id,
          decoded.userId,
          {
            courtsAvailable: 1,
        ...
        const shouldRetry =
          isRetryableWriteConflict(generationError) && attempt < maxAttempts;
        if (!shouldRetry) {
          throw generationError;
        }
        // Small linear backoff to allow competing transaction to finish.
        await new Promise((resolve) => setTimeout(resolve, attempt * 200));
      }
    }

    if (!result) {
      throw new Error("Failed to generate draw after retries");
    }
    ...
    const message = err?.message || "Failed to generate matches";
    const isHybridGenerationError = message.startsWith(
      "Hybrid round-robin generation failed:"
    );
    const hybridDetails = isHybridGenerationError
      ? message.replace("Hybrid round-robin generation failed:", "").trim()
      : "";
    return NextResponse.json(
      { 
        error: isHybridGenerationError
          ? (hybridDetails || "Hybrid round-robin generation failed")
          : "Failed to generate matches",
        ...(process.env.NODE_ENV === "development" && { details: message })
      },
      { status: isHybridGenerationError ? 400 : 500 }
    );
  }
}