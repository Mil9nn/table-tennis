...
import { warnDeprecatedModelFieldOnce } from "@/lib/modelFieldWarnings";
...
export async function GET(
  ...
  try {
    ...
    if (tournament.format === "knockout" || tournament.format === "hybrid") {
      ...
      if (hasBracket) {
        warnDeprecatedModelFieldOnce(
          "tournament.inline.bracket",
          "Reading inline tournament.bracket payload. Prefer BracketState as canonical bracket source.",
          { tournamentId: id, format: tournament.format },
        );
      }
    ...
    const resolveStandingParticipant = (standing: any) => {
      ...
      if (participant && typeof participant === "object" && participant._id) {
        ...
        if (hasDisplayName) return standing;
      }
      const participantId =
        (participant && typeof participant === "object" && participant._id
          ? participant._id.toString()
          : participant?.toString?.()) ||
        standing.participantId?.toString?.() ||
        "";
