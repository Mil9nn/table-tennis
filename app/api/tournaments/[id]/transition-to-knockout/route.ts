...
export async function POST(
  ...
  try {
    ...
    // Match GET /hybrid-status: projection-backed groups/standings often live off the
    // tournament document. Hydrate from projections and sync round.completed from
    // match documents before validating; otherwise the UI can show "ready" while POST fails.
    const rrPhase = (tournament.currentPhase || "round_robin") === "round_robin";
    if (tournament.format === "hybrid" && rrPhase) {
      const { updateRoundRobinStandings } = await import(
