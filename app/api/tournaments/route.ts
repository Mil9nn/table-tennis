...
import { normalizeTeamConfig } from "@/shared/tournament/teamConfigUtils";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const baseFields = {
      ...
      currentPhase: format === "hybrid" ? ("round_robin" as const) : undefined,
    ...
    let tournament: InstanceType<typeof TournamentTeam> | InstanceType<typeof TournamentIndividual>;

    if (category === "team") {
      const normalizedTeamConfig = normalizeTeamConfig(
        teamConfig
          ? {
              ...teamConfig,
      ...
      tournament = await TournamentTeam.create({
        ...baseFields,
        category: "team",
        teamConfig: normalizedTeamConfig,
      });
    } else {
      const processedDoublesPairs =
        matchType === "doubles" && doublesPairs
          ? (() => {
      ...
      tournament = await TournamentIndividual.create({
        ...baseFields,
        category: "individual",
        matchType: matchType || "singles",
        doublesPairs: processedDoublesPairs,
      });
    }
      ...
      if (t.groups?.length) {
        for (const group of t.groups) {
          ...
          if (group.standings?.length) {
            if (isDoublesTournament && t.doublesPairs?.length) {
              const pairMap = new Map<string, any>();
              for (const pair of t.doublesPairs) {
                if (!pair?._id) continue;
                const player1 = resolveUser(pair.player1);
              ...
              for (const pair of t.doublesPairs) {
                const player1 = resolveUser(pair.player1);
                const player2 = resolveUser(pair.player2);
                if (player1?._id) playerToPairMap.set(player1._id.toString(), pairMap.get(pair._id.toString()));
                if (player2?._id) playerToPairMap.set(player2._id.toString(), pairMap.get(pair._id.toString()));
              }
              group.standings = group.standings.map((s: any) => {
                const key = s.participant?.toString?.();
                const pair =
                  (key && pairMap.get(key)) ||
                  (key && playerToPairMap.get(key)) ||
                  null;
                return {
                  ...s,
                  participant: pair || s.participant,
                };
              });
            } else {
              group.standings = group.standings.map((s: any) => {
                ...
                return {
                  ...
                  participant: key
                    ? (isTeamTournament
                        ? teamMap.get(key) || s.participant
                        : userMap.get(key) || s.participant)
                    : s.participant,
                };
              });
            }
