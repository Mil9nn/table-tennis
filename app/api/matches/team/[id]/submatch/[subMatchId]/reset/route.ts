...
} from "@/services/match/teamRubberScoreUtils";
  ...
  try {
    ...
    if (resetType === "game") {
      ...
      if (gameIndex >= 0) {
        ...
        writeRubberTeamScores(
          subMatch.games[gameIndex],
          team1PlayerId,
          team2PlayerId,
          0,
          0
        );
        subMatch.games[gameIndex].winnerId = null;
        subMatch.games[gameIndex].status = "in_progress";
      }
      ...
      // Full submatch reset
      const { team1PlayerId, team2PlayerId } = getRubberAnchorPlayerIds(subMatch);
      subMatch.games = [createEmptyRubberGame(1, team1PlayerId, team2PlayerId)];
