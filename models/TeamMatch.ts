...
import {
  ...
  createTeamServerConfigSchema,
  TEAM_SERVER_KEY_ENUM,
  playerStatsSchema,
  teamInfoSchema,
...
const teamServerConfigSchema = createTeamServerConfigSchema();
...
const subMatchSchema = new mongoose.Schema({
  ...
  serverConfig: {
    type: teamServerConfigSchema,
    default: null,
  },

  currentServer: {
    type: String,
    enum: [...TEAM_SERVER_KEY_ENUM],
    default: null,
  },
...
TeamMatchSchema.pre('save', function(next) {
  ...
  // Custom-format rubbers may be empty at creation; configured later via POST .../submatches
...
if (mongoose.models.team) {
  TeamMatch = mongoose.models.team as mongoose.Model<ITeamMatch>;
} else if (Match.discriminators?.team) {
  TeamMatch = Match.discriminators.team as mongoose.Model<ITeamMatch>;
} else {
  TeamMatch = Match.discriminator<ITeamMatch>("team", TeamMatchSchema);
}
