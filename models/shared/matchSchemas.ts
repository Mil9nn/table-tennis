...
/** @deprecated Legacy team rubber schema (no embedded shots). TeamMatch uses `createIdBasedGameSchema`. */
...
/** Embedded shot for ID-based individual games (`side` is scoring player id). */
export function createIdBasedShotSchema() {
  return new mongoose.Schema({
    ...
    server: { type: mongoose.Schema.Types.ObjectId, ref: "User", default: null },
    originX: { type: Number, min: -100, max: 200 },
    originY: { type: Number, min: -100, max: 200 },
    landingX: { type: Number, min: 0, max: 100 },
    landingY: { type: Number, min: 0, max: 100 },
...
export function createIdBasedGameSchema() {
  const idBasedShotSchema = createIdBasedShotSchema();
  return new mongoose.Schema({
    ...
    shots: { type: [idBasedShotSchema], default: [] },
    duration: Number,
    startTime: Date,
    endTime: Date,
  });
...
/** Team rubber server keys (singles + doubles rotation). */
export const TEAM_SERVER_KEY_ENUM = [
  "team1",
  "team2",
  "team1_main",
  "team1_partner",
...
/** Server config for team submatches (position keys, not player ObjectIds). */
export function createTeamServerConfigSchema() {
  return createServerConfigSchema(["team1", "team2"], [
    "team1_main",
    "team1_partner",
    "team2_main",
    "team2_partner",
