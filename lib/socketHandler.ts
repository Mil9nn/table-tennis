...
export function initializeSocketHandlers(
  ...
  io.on("connection", (socket: SocketType) => {
    ...
    socket.on("join:match", async (data) => {
      ...
      if (!matchId) {
        console.warn(`[Socket.IO] Client ${socketId} tried to join without matchId`);
        return;
      }
      ...
      try {
        ...
        console.error(
          `[Socket.IO] Error joining room ${roomName}:`,
          error
        );
      }
    });
    ...
    socket.on("leave:match", async (data) => {
      ...
      if (!matchId) {
        console.warn(`[Socket.IO] Client ${socketId} tried to leave without matchId`);
        return;
      }
      ...
      try {
        ...
        console.log(`[Socket.IO] Client ${socketId} left room ${roomName}`);
        ...
        console.error(
          `[Socket.IO] Error leaving room ${roomName}:`,
          error
        );
      }
    });
    ...
    socket.on("disconnect", async (reason) => {
      console.log(
        `[Socket.IO] Client disconnected: ${socketId}, reason: ${reason}`
      );
      ...
      if (metadata?.matchId) {
        ...
        try {
          ...
          console.error(
            `[Socket.IO] Error handling disconnect for room ${roomName}:`,
            error
          );
        }
      }
