...
function getReceiverInfo(
  ...
  // Doubles: First try to use stored firstReceiverPlayerId from server config
  if (isDoubles && serverConfig?.firstReceiverPlayerId) {
    const firstReceiverId = serverConfig.firstReceiverPlayerId.toString();
    
    const receiverIndex = participants.findIndex(p => {
      const pid = typeof p === 'string' ? p : p._id?.toString();
      return pid === firstReceiverId;
    });
  ...
  // Fallback: if no serverConfig or firstReceiverPlayerId not found, use smart opposite side logic
  // Try to determine receiver from current game state and rotation
  const serverIndex = participants.findIndex(p => {
    const pid = typeof p === 'string' ? p : p._id?.toString();
    return pid === serverId;
  });
  ...
  // For doubles, try to be smarter about receiver selection
  // If we have game score, try to determine receiver based on serve rotation
  if (isDoubles && currentGameScore && serverConfig?.serverOrderPlayerIds?.length === 4) {
    const totalPoints = currentGameScore.side1Score + currentGameScore.side2Score;
    const serveCycle = Math.floor(totalPoints / 2);
    const currentRotationIndex = serveCycle % 4;
    ...
    if (serverRotationIndex !== -1) {
      // Receiver is next in rotation after server
      const receiverRotationIndex = (serverRotationIndex + 1) % 4;
      const receiverPlayerId = serverConfig.serverOrderPlayerIds[receiverRotationIndex]?.toString();
      
      if (receiverPlayerId) {
        const receiverIndex = participants.findIndex(p => {
          const pid = typeof p === 'string' ? p : p._id?.toString();
          return pid === receiverPlayerId;
        });
  ...
  // Final fallback: use opposite side but try to pick the correct player based on context
  // Default to first player of opposite side only when we have no other information
  const receiver = isServerSide1 ? participants[2] : participants[0];
  if (!receiver) return null;
...
export function generateFullCommentary(
  ...
  if (currentGameScore !== undefined) {
    ...
    gameScoreText = ` The game score is now <strong>${side1Score}–${side2Score}</strong>.`;
  }
