...
// Team schema for the new doubles model
const matchTeamSchema = z.object({
  players: z.array(objectIdSchema).min(1).max(2),
});
...
export const createIndividualMatchSchema = z.object({
  ...
  teams: z.array(matchTeamSchema).length(2).optional(),
...
.refine(
  (data) => {
    return data.teams || data.participants;
  },
  {
    message: "Either 'teams' or 'participants' must be provided",
    path: ["teams"],
  }
)
.refine(
  (data) => {
    if (data.matchType === "singles") {
      if (data.teams) {
        return data.teams.length === 2 &&
          data.teams[0].players.length === 1 &&
          data.teams[1].players.length === 1;
      }
      return !data.participants || data.participants.length === 2;
    }
    return true;
  },
  {
    message: "Singles matches must have exactly 2 players (1 per team)",
    path: ["participants"],
  }
)
.refine(
  (data) => {
    if (data.matchType === "doubles") {
      if (data.teams) {
        return data.teams.length === 2 &&
          data.teams[0].players.length === 2 &&
          data.teams[1].players.length === 2;
      }
      return !data.participants || data.participants.length === 4;
    }
    return true;
  },
  {
    message: "Doubles matches must have exactly 4 players (2 per team)",
    path: ["participants"],
  }
)
.refine(
  (data) => {
    const allIds = data.teams
