...
/**
 * Google sign-in — client sends the ID token from Google OAuth
 */
 ...
 * Validates OTP and new password for password reset
 */
...
export const phoneNumberSchema = z
  ...
  .refine(
    (digits) => /^[6-9]\d{9}$/.test(digits) || /^91[6-9]\d{9}$/.test(digits),
 ...
 * Password is validated server-side for email/password accounts.
 */
