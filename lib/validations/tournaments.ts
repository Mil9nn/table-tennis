...
export const hybridConfigSchema = z.object({
  ...
  qualificationMethod: z.enum(["top_n_per_group"]).optional(),
...
const teamCustomSubMatchSchema = z.object({
  matchNumber: z.coerce.number().int().min(1).max(9),
...
.refine(
  (data) => {
    if (data.category === "individual" && !data.matchType) {
      return false;
    }
    return true;
  },
  {
    message: "Individual tournaments must specify matchType (singles or doubles)",
    path: ["matchType"],
  }
)
.refine(
