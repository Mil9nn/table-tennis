...
export default function CustomFormatScorer({ match }: CustomFormatScorerProps) {
  ...
  // Determine players based on match type (singles/doubles)
  const isDoublesMatch = currentSubMatch.matchType === "doubles";

  const getPlayersForSubmatch = () => {
    if (isDoublesMatch) {
      // Get the 2 players for each team based on submatch config
      // This needs custom handling since we don't have assignments
      return {
        player1: [currentSubMatch.playerTeam1 as Participant, currentSubMatch.playerTeam1 as Participant],
        player2: [currentSubMatch.playerTeam2 as Participant, currentSubMatch.playerTeam2 as Participant],
      };
    } else {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-2">
      ...
      <Card className="shadow-none rounded-none">
        <CardHeader>
          <CardTitle>Custom Format Match</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="flex items-center gap-2 overflow-x-auto pb-2">
            ...
            {match.subMatches.map((sm, idx) => {
              ...
              const matchTypeLabel = sm.matchType === "singles" ? "S" : "D";
        ...
        <CardHeader>
          <div className="flex flex-col gap-2">
            <CardTitle className="w-full flex items-center justify-between gap-2">
              <span>
                {currentSubMatch.matchType === "singles" ? "Singles" : "Doubles"} Match {currentSubMatchIndex + 1}
              </span>
