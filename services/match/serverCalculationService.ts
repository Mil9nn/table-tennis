...
function getNextServerDoubles(
  ...
  if (isDeuce) {
    // At 10-10, each player serves one point in rotation order (ITTF doubles rule)
    const deuceStartTotalPoints = 20;
    const deuceEntryIndex =
      Math.floor(deuceStartTotalPoints / 2) % actualRotation.length;
    const serverIndex =
      (deuceEntryIndex + (totalPoints - deuceStartTotalPoints)) %
      actualRotation.length;
