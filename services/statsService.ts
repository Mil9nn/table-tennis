...
class StatsService {
  async updateIndividualMatchStats(matchId: string) {
    ...
    // Shots are now embedded in the match document, no need to merge
  ...
  private determineIndividualMatchResult(match: any): {
    ...
    if (hasTeams) {
      ...
      team0Players = isSingles
        ? [participants[0]].filter(Boolean)
        : [participants[0], participants[1]].filter(Boolean);
      team1Players = isSingles
        ? [participants[1]].filter(Boolean)
        : [participants[2], participants[3]].filter(Boolean);
    }

    // Priority 1: winnerTeamIndex
    if (match.winnerTeamIndex != null) {
      return match.winnerTeamIndex === 0
        ? { winners: team0Players, losers: team1Players }
        : { winners: team1Players, losers: team0Players };
    }

    // Priority 2: winnerId membership
    const winnerId =
    ...
    if (winnerId) {
      const team0Ids = new Set(team0Players.map((p: any) => p?._id?.toString?.() || String(p)));
      return team0Ids.has(String(winnerId))
        ? { winners: team0Players, losers: team1Players }
        : { winners: team1Players, losers: team0Players };
    }

    // Priority 3: legacy winnerSide
    if (match.winnerSide === "side1") return { winners: team0Players, losers: team1Players };
    if (match.winnerSide === "side2") return { winners: team1Players, losers: team0Players };
   ...
   */
  private getUserTeamIndex(match: any, userId: string): number {
    if (match.teams && Array.isArray(match.teams) && match.teams.length === 2) {
      const t0Ids = (match.teams[0].players || []).map((p: any) => p?._id?.toString?.() || String(p));
      if (t0Ids.includes(userId)) return 0;
      return 1;
    }
    const participants = match.participants || [];
    const isSingles = match.matchType === "singles";
    if (isSingles) {
      return participants[0]?._id?.toString() === userId ? 0 : 1;
    }
    const isInTeam0 =
      participants[0]?._id?.toString() === userId ||
      participants[1]?._id?.toString() === userId;
    return isInTeam0 ? 0 : 1;
  }
  ...
  private getIndividualUserSets(
    ...
    const teamIdx = this.getUserTeamIndex(match, userId);
    const oppIdx = teamIdx === 0 ? 1 : 0;

    // Priority 1: setsByTeam
    if (match.finalScore?.setsByTeam && match.finalScore.setsByTeam.length === 2) {
      return {
        userSets: Number(match.finalScore.setsByTeam[teamIdx] ?? 0),
        opponentSets: Number(match.finalScore.setsByTeam[oppIdx] ?? 0),
      };
    }
    ...
    // Priority 3: legacy side1Sets/side2Sets
    const userSets = teamIdx === 0
      ? match.finalScore?.side1Sets || 0
      : match.finalScore?.side2Sets || 0;
    const opponentSets = oppIdx === 0
      ? match.finalScore?.side1Sets || 0
      : match.finalScore?.side2Sets || 0;
