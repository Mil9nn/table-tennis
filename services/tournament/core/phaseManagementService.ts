...
export function validateHybridConfig(tournament: Tournament): {
  ...
  const effectiveRoundRobinUseGroups =
    Boolean(config.roundRobinUseGroups) || Boolean((tournament as any).useGroups);
  const effectiveRoundRobinNumberOfGroups =
    Number(config.roundRobinNumberOfGroups || (tournament as any).numberOfGroups || 0);
  ...
  switch (config.qualificationMethod) {
    case "top_n_per_group":
      if (!effectiveRoundRobinUseGroups) {
  ...
  if (effectiveRoundRobinUseGroups) {
    const participantCount = Array.isArray(tournament.participants)
      ? tournament.participants.filter((p: any) => p != null).length
      : 0;

    if (
      !effectiveRoundRobinNumberOfGroups ||
      effectiveRoundRobinNumberOfGroups < 2
    ) {
      errors.push("Number of groups must be at least 2");
    }
    if (
      effectiveRoundRobinNumberOfGroups &&
      effectiveRoundRobinNumberOfGroups >= participantCount
    ) {
      errors.push("Number of groups must be less than total participants");
    }
    if (
      effectiveRoundRobinNumberOfGroups &&
      participantCount < effectiveRoundRobinNumberOfGroups * 2
    ) {
      errors.push(
        `Need at least ${effectiveRoundRobinNumberOfGroups * 2} participants for ${effectiveRoundRobinNumberOfGroups} groups (minimum 2 per group)`
      );
    }
  }
