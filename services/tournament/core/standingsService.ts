...
export function calculateStandings(
  ...
  completedMatches.forEach((match) => {
    ...
    const hasTeams = (match as any).teams && Array.isArray((match as any).teams) && (match as any).teams.length === 2;

    if (hasTeams) {
      p1Id = (match as any).teams[0].players[0]?.toString();
      p2Id = (match as any).teams[1].players[0]?.toString();
    } else if (isDoublesWith4Players) {
    ...
    const setsByTeam = (match as any).finalScore?.setsByTeam;
    const setsById = match.finalScore.setsById || {};
    const p1Sets = setsByTeam ? Number(setsByTeam[0] ?? 0) : Number(setsById[p1Id] ?? 0);
    const p2Sets = setsByTeam ? Number(setsByTeam[1] ?? 0) : Number(setsById[p2Id] ?? 0);
    ...
    match.games.forEach((game) => {
      if ((game as any).scoresByTeam && (game as any).scoresByTeam.length === 2) {
        p1Points += Number((game as any).scoresByTeam[0] ?? 0);
        p2Points += Number((game as any).scoresByTeam[1] ?? 0);
      } else {
        const scoresById = game.scoresById || {};
        p1Points += Number(scoresById[p1Id] ?? 0);
        p2Points += Number(scoresById[p2Id] ?? 0);
      }
    });
    ...
    // Determine winner using canonical winner fields only.
    let p1Won = false;
    let p2Won = false;
    const wti = (match as any).winnerTeamIndex;
    if (wti != null) {
      p1Won = wti === 0;
      p2Won = wti === 1;
    } else {
...
export function getHeadToHeadRecord(
  ...
  h2hMatches.forEach((match) => {
    if (match.winnerId === participant1Id) {
      p1Wins++;
    } else if (match.winnerId === participant2Id) {
      p2Wins++;
    } else {
      draws++;
    }
