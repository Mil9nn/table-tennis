...
export class DoublesStandingsService implements IStandingsService {
  ...
  calculateStandings(
    ...
    completedMatches.forEach((match) => {
      ...
      // Calculate sets and points using team-indexed fields when available
      const setsByTeam = (match as any).finalScore?.setsByTeam;
      const setsById = match.finalScore?.setsById || {};
      const team1Sets = setsByTeam ? Number(setsByTeam[0] ?? 0) : Number(setsById[team1Id] ?? 0);
      const team2Sets = setsByTeam ? Number(setsByTeam[1] ?? 0) : Number(setsById[team2Id] ?? 0);
      ...
      match.games.forEach((game) => {
        if ((game as any).scoresByTeam && (game as any).scoresByTeam.length === 2) {
          team1Points += Number((game as any).scoresByTeam[0] ?? 0);
          team2Points += Number((game as any).scoresByTeam[1] ?? 0);
        } else {
          const scoresById = game.scoresById || {};
          team1Points += Number(scoresById[team1Id] ?? 0);
          team2Points += Number(scoresById[team2Id] ?? 0);
        }
      });
      ...
      if (wti != null) {
        ...
        t1Won = winnerId === team1Id;
        t2Won = winnerId === team2Id;
      }
