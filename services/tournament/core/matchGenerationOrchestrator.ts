...
export class MatchGenerationOrchestrator {
  ...
  async generateTournamentDraw(
    ...
    const runGeneration = async (
      ...
      if (tournament.format === "hybrid") {
        ...
        const hybridResult = await generateHybridRoundRobinPhase(tournament, {
        ...
        if (!hybridResult.success) {
          const details =
            (hybridResult.errors && hybridResult.errors.length > 0
              ? hybridResult.errors.join(" | ")
              : hybridResult.message) || "Unknown hybrid generation failure";
          throw new Error(`Hybrid round-robin generation failed: ${details}`);
        }

        // Persist projection-backed round-robin data.
        // Hybrid generation mutates in-memory tournament groups/standings, but those
        // fields are no longer persisted on tournament schemas.
        const category = (tournament as any).category === "team" ? "team" : "individual";
        const phase = (tournament as any).currentPhase;
        const groups = Array.isArray((tournament as any).groups)
          ? (tournament as any).groups
          : [];
        const standings = Array.isArray((tournament as any).standings)
    ...
    const tournamentForRouting = await this.tournamentRepo.findById(tournamentId);
    if (!tournamentForRouting) {
      throw new Error(`Tournament ${tournamentId} not found`);
    }

    // Round-robin generation can create many match documents and may exceed
    // MongoDB transaction lifetime limits in dev/Atlas tiers. Execute these
    // formats without an outer multi-document transaction.
    if (
      tournamentForRouting.format === "hybrid" ||
      tournamentForRouting.format === "round_robin"
    ) {
      ...
      for (const pairing of round.matches) {
        if (isTeamCategory) {
          ...
          const subMatches = this.generateSubMatches(
            ...
            team2Data,
            setsPerSubMatch,
            teamTournament.teamConfig?.customSubMatches
          );
  ...
  private generateSubMatches(
    ...
    customSubMatches?: Array<{ matchNumber: number; matchType: "singles" | "doubles" }>
  ): any[] {
    ...
    if (matchFormat === "five_singles") {
    ...
    } else if (matchFormat === "custom" && customSubMatches?.length) {
      const playerA = team1PositionMap.get("A") || null;
      const playerB = team1PositionMap.get("B") || null;
      const playerC = team1PositionMap.get("C") || null;
      const playerX = team2PositionMap.get("X") || null;
      const playerY = team2PositionMap.get("Y") || null;
      const playerZ = team2PositionMap.get("Z") || null;

      const singlesOrder: Array<[string | null, string | null]> = [
        [playerA, playerX],
        [playerB, playerY],
        [playerC, playerZ],
        [playerA, playerY],
      ...
      const sorted = [...customSubMatches].sort(
        (a, b) => a.matchNumber - b.matchNumber
      );

      for (const rubber of sorted) {
        if (rubber.matchType === "doubles") {
          subMatches.push({
            matchNumber: rubber.matchNumber,
            matchType: "doubles",
            playerTeam1: playerA && playerB ? [playerA, playerB] : [],
            playerTeam2: playerX && playerY ? [playerX, playerY] : [],
            numberOfGames: setsPerSubMatch,
          ...
          const [p1, p2] = singlesOrder[singlesIdx % singlesOrder.length];
          singlesIdx += 1;
          subMatches.push({
            matchNumber: rubber.matchNumber,
            matchType: "singles",
