...
export async function updateTournamentAfterMatch(match: any) {
  ...
  // Groups/standings are projection-backed; hydrate them before applying
  // round-robin status/standings updates, especially for newly created tournaments.
  const hydratedTournament = tournament.toObject ? tournament.toObject() : tournament;
  await loadAndApplyProjectedTournamentData(
    String((tournament as any)._id),
    hydratedTournament
  );
  (tournament as any).groups = Array.isArray((hydratedTournament as any).groups)
    ? (hydratedTournament as any).groups
    : [];
  (tournament as any).standings = Array.isArray((hydratedTournament as any).standings)
    ? (hydratedTournament as any).standings
    : [];
...
export async function updateRoundRobinStandings(tournament: any) {
  // Projection-backed groups/standings are not persisted on tournament schemas.
  // Always hydrate them before recalculation to avoid treating grouped tournaments
  // as empty single-round-robin and wiping projection docs.
  if (tournament?._id) {
    const projected = tournament.toObject ? tournament.toObject() : { ...(tournament as any) };
    await loadAndApplyProjectedTournamentData(String(tournament._id), projected);
    if (Array.isArray((projected as any).groups)) {
      tournament.groups = (projected as any).groups;
    }
    if (Array.isArray((projected as any).standings)) {
      tournament.standings = (projected as any).standings;
    }
  }
